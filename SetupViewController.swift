...
internal class SetupViewController: UIViewController {
   ...
   private var isArmOnAndOFF: Bool {
        get {
    ...
    private func updateOnOffButton() {
        onOffButton.backgroundColor = isArmOnAndOFF ? .red : UIColor.brand.theme
        onOffButton.setTitle("\(isArmOnAndOFF ? "Turn off" : "Turn on")", for: .normal)
    }
